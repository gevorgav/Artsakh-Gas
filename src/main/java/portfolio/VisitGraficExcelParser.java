...
public class VisitGraficExcelParser {
	...
	public void exportVisitGrafic() throws IOException, InvalidFormatException {
		...
		try(XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(file))) {
			...
			float percent5 = plannedC + plannedCCompanies != 0 ? (float)(visitedClientsCount + visitedCompaniesCount)/(plannedC + plannedCCompanies) : 0f;
			row.getCell(5).setCellValue(percent5);
			...
			float percent9 = plannedCSemi + plannedCSemiCompanies != 0 ? (float)(visitedClientsCountSemi + visitedCompaniesCountSemi) / (plannedCSemi + plannedCSemiCompanies) : 0f;
			row.getCell(9).setCellValue(percent9);
...
//        sheet.addMergedRegion(CellRangeAddress.valueOf("A12:B12"));
			...
			for (int i = 0; i < sections.size(); i++) {
				...
				percent5 = plannedC + plannedCCompanies != 0 ? (float)(visitedClientsCount + visitedCompaniesCount)/(plannedC + plannedCCompanies) : 0f;
				row.getCell(5).setCellValue(percent5);
				...
				percent9 = plannedCSemi + plannedCSemiCompanies != 0 ? (float)(visitedClientsCountSemi + visitedCompaniesCountSemi)/(plannedCSemi + plannedCSemiCompanies) : 0f;
				row.getCell(9).setCellValue(percent9);
			...
			try (FileOutputStream out = new FileOutputStream(portfolioForm.getFile2UploadUrl())) {
				workbook.write(out);
			}
		}
	}
	...
	public void setCellStyleAlign(Cell cell, XSSFWorkbook workbook, HorizontalAlignment cellStyle) {
		XSSFCellStyle style = workbook.createCellStyle();
	...
	public void exportVisitGraficAllRegions() throws IOException, InvalidFormatException {
		ClassLoader classLoader = getClass().getClassLoader();
		File file = new File(portfolioForm.getTemaplate3Url());
		try(XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(file))) {
			...
			Row row;
			int index = 9;
			for (Region region : portfolioForm.getCache().getRegions()) {
...
//			row.getCell(13).setCellValue(regionName + "ի ԱՇՏ");
				...
				createCell(row, 1);
				setCellStyle(row.getCell(1), workbook, (short) 10);
				sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("A%s:B%s", index + 1, index + 1)));
				setCellStyleAlign(row.getCell(0), workbook, HorizontalAlignment.CENTER);
				...
				createCell(row, 15);
				setCellStyle(row.getCell(15), workbook, (short) 10);
				row.getCell(15).setCellValue(violationCodesCountSemi + violationCodesCountSemiCompanies);

				createCell(row, 16);
				setCellStyle(row.getCell(16), workbook, (short) 10);
				row.getCell(16).setCellValue(violationCodesCountSemi + violationCodesCountSemiCompanies);

				createCell(row, 17);
				setCellStyle(row.getCell(17), workbook, (short) 10);
				row.getCell(17).setCellValue(violationCodesCountSemi + violationCodesCountSemiCompanies);

				row = createRow(sheet, index + 1);
				row.setHeight((short) 400);
...
//		sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("B%s:D%s", rowIndex, rowIndex)));
			...
			// Write the output to the file
			try (FileOutputStream out = new FileOutputStream(portfolioForm.getFile3UploadUrl())) {
				workbook.write(out);
			}
		}
